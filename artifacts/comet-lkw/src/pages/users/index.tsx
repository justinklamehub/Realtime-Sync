...
export default function UsersPage() {
  ...
  const getRoleBadge = (role: string) => {
    if (role.startsWith("comet_")) {
      return <Badge className="bg-primary/10 text-primary border-none hover:bg-primary/20">{role}</Badge>;
    }
    return <Badge variant="secondary" className="bg-slate-100 text-slate-700 border-none">{role}</Badge>;
  };
            ...
            {isLoading ? (
              ...
              users.map((user) => (
                <TableRow key={user.id}>
                  ...
                  <TableCell className="text-slate-700 font-medium">{user.speditionName || "COMET (Intern)"}</TableCell>
